...
const getChatters = async (userId) => {
  try {
    ...
    // selected all users and join with previous tables "all chats" where userId which we passed and chats id
    // select "u"."id" from "Users" as u
    // gives chats where userId chatting
    ...
    return result.length > 0 ? result.map((v) => v.userId) : [];
  } catch (err) {
    console.log(err);
    return [];
  }
